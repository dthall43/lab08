...
bool Maze::traverse(int row, int col)
{
   ...
   if (maze->getElement(row,col)!=WALL && maze->getElement(row,col)!=TRIED)
   {
      ...
      else
      {
		...
		bool b1 = traverse(row,col+1);
		bool b2 = traverse(row+1,col);
		bool b3 = traverse(row-1,col);
		bool b4 = traverse(row,col-1);
		if(b1||b2||b3||b4){
			done = true;
		}
